...
<div class="container mt-4 ">
     <div class="row">
     <div class="col-md-6">
             <a href="admin-year.php?deptid=<?=$deptid?>" class="text-info bg-light p-2">YEAR <?= $yearname ?></a> &gt; <a href="admin-month.php?deptid=<?=$deptid?>&amp;yearid=<?= $yearid?>" class="text-info text-uppercase bg-light p-2"> MONTH <?= $monthname ?></a> &gt; <a href="" class="text-info bg-light p-2 text-uppercase"><?= $prgmtype?> PROGRAMME</a>
        </div>
        <div class="col-md-6">
            ...
            <div class="container">
                    <div class="card">
                        <div class="card-header">
                             <h6 class="text-uppercase"><?= $prgmtype ?> Programme</h6>
                        </div>
                                                    ...
                                                    <tr class="">
                                                       ...
                                                       <q><?= $row['title'] ?></q>
                                                        ...
                                                        <td><div class="btn-group"><a href="inserting/otherprogramme.php?deptid=<?php $deptid =$_GET['deptid']; if(isset($deptid) ) 
															...
															echo $monthid; else die('Some Error') ?>&amp;programme=constructive&amp;programmedid=<?= $row['prgmid'] ?>" class="btn btn-sm btn-primary">Edit</a> <button class="btn btn-sm btn-danger delete" value="<?= $row['prgmid'] ?>" > Delete</button> <a href="" class="btn btn-sm btn-success">Report</a> 
                                                             <a href="inserting/viewprogramme.php?deptid=<?php $deptid =$_GET['deptid']; if(isset($deptid) ) 
															echo $deptid; else die('Some Error') ?>&amp;yearid=<?php $yearid =$_GET['yearid']; if(isset($deptid) ) 
															echo $yearid; else die('Some Error') ?>&amp;monthid=<?php $monthid =$_GET['monthid']; if(isset($monthid) ) 
															echo $monthid; else die('Some Error') ?>&amp;programme=constructive &amp;programmedid=<?= $row['prgmid'] ?>" class="btn btn-sm btn-warning">View</a></div></td>
                                                        <td><?= $row['userid']?></td>
                                                        </tr>
                        ...
                        <div class="card-footer p-0 bg-primary" >
                            <a class="btn d-block w-100 mx-auto text-white" href="inserting/otherprogramme.php?deptid=<?php $deptid =$_GET['deptid']; if(isset($deptid) ) 
															echo $deptid; else die('Some Error') ?>&amp;yearid=<?php $yearid =$_GET['yearid']; if(isset($deptid) ) 
															echo $yearid; else die('Some Error') ?>&amp;monthid=<?php $monthid =$_GET['monthid']; if(isset($monthid) ) 
															echo $monthid; else die('Some Error') ?>&amp;programme=<?= $prgmtype ?>">ADD</a>
